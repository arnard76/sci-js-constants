...
exports.exact_values = exports.ConstantDeprecatedWarning = exports._obsolete_constants = exports.physical_constants = void 0;
exports.value = value;
exports.unit = unit;
...
function parse_constants_2002to2014(d) {
    const constants = {};
    for (const line of d.split('\n')) {
        const name = line.slice(0, 55).trimEnd();
        const val = parseFloat(line.slice(55, 77).replaceAll(' ', '').replaceAll('...', ''));
        const uncert = parseFloat(line.slice(77, 99).replaceAll(' ', '').replaceAll('(exact)', '0'));
        const units = line.slice(99).trimEnd();
        constants[name] = [val, units, uncert];
    }
    return constants;
}
function parse_constants_2018toXXXX(d) {
    const constants = {};
    for (const line of d.split('\n')) {
        const name = line.slice(0, 60).trimEnd();
        const val = parseFloat(line.slice(60, 85).replaceAll(' ', '').replaceAll('...', ''));
        const uncert = parseFloat(line.slice(85, 110).replaceAll(' ', '').replaceAll('(exact)', '0'));
        const units = line.slice(110).trimEnd();
        constants[name] = [val, units, uncert];
    }
    return constants;
}
const _physical_constants_2002 = parse_constants_2002to2014(txt2002);
const _physical_constants_2006 = parse_constants_2002to2014(txt2006);
const _physical_constants_2010 = parse_constants_2002to2014(txt2010);
const _physical_constants_2014 = parse_constants_2002to2014(txt2014);
const _physical_constants_2018 = parse_constants_2018toXXXX(txt2018);
exports.physical_constants = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, _physical_constants_2002), _physical_constants_2006), _physical_constants_2010), _physical_constants_2014), _physical_constants_2018);
const _current_constants = _physical_constants_2018;
const _current_codata = 'CODATA 2018';
exports._obsolete_constants = {};
for (const k in exports.physical_constants) {
    if (!Object.keys(_current_constants).includes(k))
        exports._obsolete_constants[k] = true;
}
const _aliases = {};
for (const k in _physical_constants_2002) {
    if (k.includes('magn.')) {
        _aliases[k] = k.replace('magn.', 'mag.');
    }
}
for (const k in _physical_constants_2006) {
    if (k.includes('momentum')) {
        _aliases[k] = k.replace('momentum', 'mom.um');
    }
}
for (const k in _physical_constants_2018) {
    if (k.includes('momentum')) {
        _aliases[k] = k.replace('momentum', 'mom.um');
    }
}
...
class ConstantDeprecatedWarning {
}
exports.ConstantDeprecatedWarning = ConstantDeprecatedWarning;
function _check_obsolete(key) {
    if (key in exports._obsolete_constants && !(key in _aliases)) {
        console.warn(`Constant '${key}' is not in current ${_current_codata} data set`, ConstantDeprecatedWarning);
    }
}
function value(key) {
    _check_obsolete(key);
    return exports.physical_constants[key][0];
}
function unit(key) {
    _check_obsolete(key);
    return exports.physical_constants[key][1];
}
function precision(key) {
    _check_obsolete(key);
    return exports.physical_constants[key][2] / exports.physical_constants[key][0];
}
function find(sub = null, disp = false) {
    let result = Object.keys(_current_constants);
    if (sub) {
        result = result.filter((key) => key.toLowerCase().includes(sub.toLowerCase()));
    }
    result.sort();
    if (disp) {
...
const c = value('speed of light in vacuum');
exports.exact_values = {
...
for (const key in exports.exact_values) {
    const val = exports.physical_constants[key][0];
    if (Math.abs(exports.exact_values[key][0] - val) / val > 1e-9) {
        throw Error('Constants.codata: exact values too far off.');
    }
    if (exports.exact_values[key][2] == 0 && exports.physical_constants[key][2] != 0) {
        throw Error('Constants.codata: value not exact');
    }
}
exports.physical_constants = Object.assign(Object.assign({}, exports.physical_constants), exports.exact_values);
const _tested_keys = [
...
for (const [k, v] of Object.entries(_aliases)) {
    if (v in _current_constants || v in _tested_keys) {
        exports.physical_constants[k] = exports.physical_constants[v];
    }
    else {
        delete _aliases[k];
    }
