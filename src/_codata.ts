...
export let physical_constants: { [constantName: string]: [number, string, number] } = {};

function parse_constants_2002to2014(d: string): typeof physical_constants {
	const constants: typeof physical_constants = {};
...
function parse_constants_2018toXXXX(d: string): typeof physical_constants {
	const constants: typeof physical_constants = {};
	for (const line of d.split('\n')) {
...
export const _obsolete_constants: { [constantName: string]: true } = {};
for (const k in physical_constants) {
	if (!Object.keys(_current_constants).includes(k)) _obsolete_constants[k] = true;
}

// generate some additional aliases
const _aliases: { [constantName: string]: string } = {};
...
export class ConstantDeprecatedWarning {
	/**
	 * Accessing a constant no longer in current CODATA data set
	 */
}

function _check_obsolete(key: string): void {
	if (key in _obsolete_constants && !(key in _aliases)) {
		console.warn(
			`Constant '${key}' is not in current ${_current_codata} data set`,
...
export function unit(key: string): string {
	/**
...
export function precision(key: string): number {
	/** 
...
export function find(sub: string | null = null, disp = false) {
	/**
...
export const exact_values: typeof physical_constants = {
...
for (const key in exact_values) {
	const val = physical_constants[key][0];
	if (Math.abs(exact_values[key][0] - val) / val > 1e-9) {
		throw Error('Constants.codata: exact values too far off.');
	}
	if (exact_values[key][2] == 0 && physical_constants[key][2] != 0) {
		throw Error('Constants.codata: value not exact');
	}
}

physical_constants = { ...physical_constants, ...exact_values };
